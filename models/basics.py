...
class SpectralConv1d(nn.Module):
    ...
    def forward(self, x):
        ...
        # Combine Hartley and flipped-Hartley
        cos_part = x_ht
        sin_part = x_ht_flip
        ...
        # Multiply Hartley modes (convolution)
        out_ht = torch.zeros(batchsize, self.out_channels, x.size(-1)//2 + 1, device=x.device)
        out_ht[:, :, :self.modes1] = compl_mul1d(cos_part[:, :, :self.modes1], self.weights1)

        # Inverse Hartley Transform (back to physical space)
        x = idht(out_ht)

        # Reshape `phase` to match `x`'s size before combining
