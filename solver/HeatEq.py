...
class HeatEq1D():
    ...
    def heat_calc_RHS(self, u):
        u_xx = self.Dxx(u)
        u_RHS = self.alpha * u_xx
        return u_RHS
    ...
    def heat_rk4(self, u, t=0):
        u_RHS1 = self.heat_calc_RHS(u)
        t1 = t + 0.5 * self.dt
        u1 = self.update_field(u, u_RHS1, step_frac=0.5)
        
        u_RHS2 = self.heat_calc_RHS(u1)
        t2 = t + 0.5 * self.dt
        u2 = self.update_field(u, u_RHS2, step_frac=0.5)
        
        u_RHS3 = self.heat_calc_RHS(u2)
        t3 = t + self.dt
        u3 = self.update_field(u, u_RHS3, step_frac=1.0)
        
        u_RHS4 = self.heat_calc_RHS(u3)
    ...
    def heat_driver(self, u0, save_interval=10, plot_interval=0):
        self.u0 = u0[:self.Nx]
        self.u = self.u0
        self.t = 0
        self.it = 0
        ...
        while self.t < self.tend:
            self.u, self.t = self.heat_rk4(self.u, self.t)
...
class HeatEq3D():
    def __init__(self,
        ...
        z = torch.linspace(zmin, zmax, Nz + 1)[:-1]
        self.x = x
        self.y = y
        self.z = z
        self.dx = x[1] - x[0]
        ...
        self.u0 = torch.zeros_like(self.u)
        self.dt = dt
        self.tend = tend
        self.t = 0
        self.it = 0
    ...
    def heat_calc_RHS(self, u):
        ...
        u_RHS = self.alpha * (u_xx + u_yy + u_zz)
        return u_RHS
    ...
    def heat_driver(self, u0, save_interval=10, plot_interval=0):
        ...
        if plot_interval != 0 and self.it % plot_interval == 0:
            self.plot_data(vmin=-1, vmax=1, title='u')
        if save_interval != 0 and self.it % save_interval == 0:
            self.U.append(self.u)
            self.T.append(self.t)
